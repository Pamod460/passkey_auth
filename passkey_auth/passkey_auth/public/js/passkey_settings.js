...
(function () {
    ...
    async function loadPasskeyList() {
        const $container = $("#passkey-list-container");
        try {
            const result = await utils.apiCallAsync("list_credentials", {});
            ...
            credentials.forEach(function (cred) {
                const lastUsed = cred.last_used_at ? frappe.datetime.str_to_user(cred.last_used_at) : __("Never");
                const created = cred.created_at ? frappe.datetime.str_to_user(cred.last_used_at) : "";
                const synced = cred.backed_up ? " | " + __("Synced") : "";
                const badge = cred.enabled ? '<span class="badge badge-success">' + __("Active") + "</span>" : '<span class="badge badge-danger">' + __("Disabled") + "</span>";
    ...
    function handleRenamePasskey(credentialName, currentNickname) {
        frappe.prompt({ label: __("Passkey Name"), fieldname: "new_nickname", fieldtype: "Data", reqd: 1, default: currentNickname },
            async function (values) {
                const result = await utils.apiCallAsync("rename_credential", { credential_name: credentialName, nickname: values.new_nickname });
                if (result.success) {
    ...
    function handleRevokePasskey(credentialName) {
        frappe.confirm(__("Revoke this passkey?"),
            async function () {
                const result = await utils.apiCallAsync("revoke_credential", { credential_name: credentialName });
    ...
    function handleDeletePasskey(credentialName) {
        frappe.confirm(__("Permanently delete this passkey?"),
            async function () {
                const result = await utils.apiCallAsync("delete_credential", { credential_name: credentialName });
